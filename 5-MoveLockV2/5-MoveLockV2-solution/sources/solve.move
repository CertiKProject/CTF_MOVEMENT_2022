module attack::solve5 {
    ...
    fun init_module(creator: &signer) {
        assert!(signer::address_of(creator) == @attack, ERR_NOT_ADMIN);
    ...
    // [*] Public functions
    //
    public fun unlock() : u128 acquires Counter {
        ...
        let x : u64 = gen_number() % 0xff; // seed
    ...
    fun increment(): u64 acquires Counter {
        let c_ref = &mut borrow_global_mut<Counter>(@attack).value;
        *c_ref = *c_ref + 1;
        *c_ref
    }
    ...
    fun encrypt_string(plaintext : vector<u8>) : vector<u8> {
        ...
        let key : vector<u8> = transaction_context::get_script_hash(); //@note what is this
        aptos_framework::debug::print(&key);
        ...
        // aptos_framework::debug::print(&ciphertext);
        ciphertext
    }

    public entry fun solve(attacker :&signer) acquires Counter{
        let res = unlock();
        move_lock::unlock(attacker, res);
    } 
